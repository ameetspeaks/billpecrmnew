...
@slot('title') Edit New User @endslot
@slot('subTitle') Edit User @endslot
@slot('content')
<form class="row forms-sample w-full" action="{{ route('admin.user.update') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group col-4 err_phone">
        ...
        <select class="form-control h-11" name="role_type">
            ...
            @foreach ($roles as $role)
                <option value="{{ $role->id }}" {{$user->role_type == $role->id ? 'selected' : ''}}>{{ $role->name }}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-4 err_phone">
        ...
        <input type="number" class="form-control" name="whatsapp_no" placeholder="Whatsapp number" value="{{$user->whatsapp_no}}" required>
    ...
    <div class="form-group col-4 err_aadhar">
        ...
        <input type="text" class="form-control" name="aadhar_number" placeholder="Aaddar Number" value="{{$user->aadhar_number}}">
        <span class="text-xs text-red-500 mt-2 err_aadhar"></span>
    </div>
   
    <div class="form-group col-4 err_driving_licence">
        <label>Driving Licence</label>
        <input type="password" class="form-control" name="driving_licence" placeholder="Driving Licence" value="{{$user->driving_licence}}">
