...
<form action="{{route('admin.tutorial.update')}}" class="row forms-sample w-full" name="sendNotificationsForm" id="sendNotificationsForm" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group col-md-6 col-12">
        ...
        <select class="form-control select"  name="modules_id[]" id="modules_id" multiple required>
            ...
            <?php 
            $modulesarray = explode(',',$tutorial->module_id);
            ?>
    ...
    <div class="form-group col-md-6 col-12">
        ...
        <input type="text" name="title"  class="form-control" id="title" placeholder="Title" value="{{$tutorial->title}}" required>
    </div>

    <div class="form-group col-md-6 col-12">
        <label><b>Description</b></label>
        <textarea type="text" name="discription" placeholder="Description"  class="form-control" id="discription" required>{{$tutorial->discription}}</textarea>
    </div>
    
    <div class="form-group col-md-6 col-12">
        <label><b>Video Url</b></label>
        <input type="text" name="url" placeholder="Url"  class="form-control" id="url" value="{{$tutorial->video_url}}" required>
    </div>
