...
@slot('title') Edit module @endslot

@slot('subTitle') edit module detail @endslot
@slot('content')
<form class="row forms-sample w-full" action="{{ route('admin.module.update') }}" method="POST">
    @csrf
    ...
    <div class="form-group col-6 err_module">
        ...
        <select class="form-control h-11" name="store_type_id" required>
            ...
            <option value="{{ $StoreType->id }}" {{$module->store_type_id == $StoreType->id ? 'selected' : ''}}>{{ $StoreType->name }}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-6 err_name">
        ...
        <input type="text" name="name" class="form-control" value="{{$module->name}}" placeholder="System Module Name">
