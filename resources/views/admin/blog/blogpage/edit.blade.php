...
<form action="{{ route('admin.blog.update') }}" method="post" enctype="multipart/form-data">                   
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
            ...
            <select class="form-control" name="category_id" id="category_id"
                ...
                @foreach ($categoties as $item)
                    <option value="{{ $item->id }}" {{$blog->blog_category_id == $item->id ? 'selected' : ''}}>{{ $item->name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="md:col-start-1 md:col-end-4">
            ...
            <input name="meta_title" id="meta_title" value="{{$blog->meta_title}}"
                class="w-full border border-gray-400 p-1 bg-white rounded focus:outline-none"
                required />
        </div>
        <div class="md:col-start-1 md:col-end-4">
            <label for="meta_desc">Meta Description</label>
            <input name="meta_desc" id="meta_desc" value="{{$blog->meta_description}}"
                class="w-full border border-gray-400 p-1 bg-white rounded focus:outline-none"
                required />
        </div>
        <div class="md:col-start-1 md:col-end-4">
            <label for="meta_keywords">Meta Keywords</label>
            <input name="meta_keywords" id="meta_keywords" value="{{$blog->meta_keyword}}"
                class="w-full border border-gray-400 p-1 bg-white rounded focus:outline-none"
                required />
        </div>
        ...
        <div class="md:col-start-1 md:col-end-4">
            ...
            <textarea name="post" id="summernote" placeholder="Write post here..."
                class="w-full border border-gray-400 p-1 bg-white rounded focus:outline-none">{{$blog->post}}</textarea>
        </div>
