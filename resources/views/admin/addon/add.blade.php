...
@slot('title') Add Addon Item @endslot

@slot('subTitle') add Addon detail @endslot
@slot('content')
<form class="row forms-sample w-full" action="{{ route('admin.addon.store') }}"  method="POST"  enctype='multipart/form-data'>
    @csrf
    ...
    <div class="form-group col-4 err_package_price">
        <label>Price</label>
        <input type="number" name="price" class="form-control" placeholder="00000">
