...
@slot('subTitle') Homepage Video Edit @endslot
@slot('content')

<form action="{{route('admin.homepagevideo.update')}}" class="row forms-sample w-full" name="sendNotificationsForm" id="sendNotificationsForm" method="POST" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="homepage_video_id" value="{{$homepageVideo->id}}">
    <div class="form-group col-md-6 col-12">
        ...
        <select class="form-control select"  name="modules_id[]" multiple required>
            ...
            <?php 
            $modulesarray = explode(',',$homepageVideo->module_id);
            ?>
    ...
    <div class="form-group col-md-6 col-12">
        ...
        <select class="form-control h-11" id="package_type" name="package_type" required>
            <option value="both" {{$homepageVideo->package_type == 'both' ? 'selected' : ''}}>Both</option>
            <option value="trial" {{$homepageVideo->package_type == 'trial' ? 'selected' : ''}}>Trial</option>
            <option value="paid" {{$homepageVideo->package_type == 'paid' ? 'selected' : ''}}>Paid</option>
        </select>
    </div>
    ...
    <div class="form-group col-md-6 col-12">
        ...
        <select class="form-control h-11" id="module_condition" name="module_condition" required>
            ...
            <option value="0" {{$homepageVideo->module_condition == '0' ? 'selected' : ''}}>First Time</option>
            <option value="1" {{$homepageVideo->module_condition == '1' ? 'selected' : ''}}>Marketing</option>
        </select>
    </div>
    ...
    <div class="form-group col-md-6 col-12 err_image">
        ...
        <input type="text" name="homepage_video_image_link"  class="form-control" value="{{$homepageVideo->homepage_video_image}}">
    </div>
