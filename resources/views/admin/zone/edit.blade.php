...
@slot('title') Edit Zone @endslot

@slot('subTitle') edit Zone detail @endslot
@slot('content')
<form class="row forms-sample w-full" action="{{ route('admin.zone.update') }}" name="moduleData" method="POST">
    @csrf
    <input type="hidden" name="zone_id" value="{{$zone->id}}">
    <div class="form-group col-6 err_name">
        <label>Zone Name</label>
        <input type="text" name="name" class="form-control" placeholder="Zone Name" value="{{$zone->name}}">
    ...
    <div class="mt-4">
        ...
        <a href="{{ route('admin.zone.index') }}"><div class="btn btn-light">Cancel</div></a>
    </div>
</form>
@endslot
@slot('script')
...
<script>
    ...
    function initialize() {
        var zone = {!! json_encode($zone->map_details) !!};
