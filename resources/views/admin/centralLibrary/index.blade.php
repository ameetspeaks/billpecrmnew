...
@slot('title') CentralLibrary Product @endslot
@slot('subTitle') CentralLibrary list @endslot
@slot('content')
...
<div class="text-right col-12 d-flex justify-end gap-1">
    ...
    <a href="{{route('admin.centralLibrary.add')}}">
    ...
    <a href="{{ route('admin.centralLibrary.export') }}"><button type="button" class="btn btn-outline-secondary btn-fw  text-capitalize">Export</button></a>
</div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <div class="modal-body">
            <form class="row forms-sample w-full" action="{{ route('admin.centralLibrary.exportByModule') }}" name="moduleData" method="POST">
                @csrf
...
<script>
    $(function () {
        var table = $('.table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.centralLibrary.getDatatable') }}",
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        return ' <a href=" {{ url('admin/editCentralProduct') }}/' + data +'" " ><button class="btn btn-success btn-sm">Edit<button></a> <a href="#" ><button class="btn btn-primary btn-sm" id="remove" data-id="' + data +'">Delete<button></a>';
    ...
    //     var url = '{{ route('admin.centralLibrary.changeStatus',["id" => ":id"]) }}';
    ...
    $(document).on('click', '#remove', function(){
        ...
        Swal.fire({
            ...
            if(result.value){
                ...
                var url = '{{ route('admin.centralLibrary.delete',["id" => ":id"]) }}';
                url = url.replace(':id', id);
                $.ajax({
                    type: 'GET',
                    url: url,
    ...
    $("#importFileForm").submit(function(e){
        ...
        var url = '{{route('admin.centralLibrary.import')}}';
        $("#hideSpinner").show();
        $.ajax({
            type: 'POST',
            url: url,
