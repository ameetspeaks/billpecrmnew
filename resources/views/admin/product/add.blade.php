...
<form class="row forms-sample w-full" action="{{ route('admin.product.store') }}"  method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group col-md-6 col-12 err_store">
        ...
        <select class="form-control h-11 cursor-pointer" id="changeStore" name="store_id">
            ...
            @foreach ($stores as $store)
                <option data-module_id="{{$store->module_id}}" value="{{ $store->id }}">{{ $store->shop_name }}</option>
            @endforeach
        </select>
