...
@slot('title') Edit Sub Zone @endslot

@slot('subTitle') edit sub Zone detail @endslot
@slot('content')
<form class="row forms-sample w-full" action="{{ route('admin.subzone.update') }}" name="moduleData" method="POST">
    @csrf
    ...
    <div class="form-group col-6 err_zone">
        ...
        <select class="form-control" name="zone_id" required>
            ...
            <option value="{{$zone->id}}" {{$subZone->zone_id == $zone->id ? 'selected' : ''}}>{{$zone->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-6 err_name">
        ...
        <input type="text" name="name" class="form-control" placeholder="Sub Zone Name" value="{{$subZone->name}}" required>
    ...
    <div class="form-group col-6 err_zone">
        ...
        <select class="form-control select" name="store_id[]" multiple required>
            ...
            @foreach ($stores as $store)
                <option value="{{ $store->id }}" {{in_array($store->id,$storesarray) ? "selected" : ""}}>{{ $store->shop_name }}</option>
            @endforeach
        </select>
        <span class="text-xs text-red-500 mt-2 err_zone"></span>
    </div>
