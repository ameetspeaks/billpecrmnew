...
@slot('title') Edit Customer Coupan @endslot

@slot('subTitle') Edit Customer Coupan detail @endslot
@slot('content')
...
<form class="row forms-sample w-full" action="{{ route('admin.charges.update') }}"  method="POST" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="chargeID" value="{{$charge->id}}">
    <div class="form-group col-6 err_name">
        <label>Name</label>
        <input type="text" name="name" class="form-control" placeholder="Name" value="{{$charge->name}}" required>
        <span class="text-xs text-red-500 mt-2 err_name"></span>
    </div>

    <div class="form-group col-6 err_amount">
        <label>Amount</label>
        <input type="text" name="amount" class="form-control" placeholder="Amount" value="{{$charge->amount}}" required>
        <span class="text-xs text-red-500 mt-2 err_amount"></span>
    </div>

    <div class="form-group col-6 err_minimum_cart_value">
        <label>Minimum Cart Value</label>
        <input type="text" name="minimum_cart_value" value="{{$charge->minimum_cart_value}}" class="form-control" placeholder="Minimum Cart Value" required>
        <span class="text-xs text-red-500 mt-2 err_minimum_cart_value"></span>
    </div>

    <div class="form-group col-6 err_start_time">
        <label>Start Time</label>
        <input type="time" name="start_time" value="{{$charge->start_time}}"  class="form-control" placeholder="Start Time">
        <span class="text-xs text-red-500 mt-2 err_start_time"></span>
    </div>

    <div class="form-group col-6 err_end_time">
        <label>End Time</label>
        <input type="time" name="end_time"  value="{{$charge->end_time}}" class="form-control" placeholder="End Time">
    ...
    <div class="form-group col-6 err_occurring">
        ...
        <select class="form-control" name="occurring" id="occurring">
            ...
            <option value="once" {{$charge->occurring == 'once' ? 'selected' : ''}}>Once</option>
            <option value="daily" {{$charge->occurring == 'daily' ? 'selected' : ''}}>Daily</option>
            <option value="permanent" {{$charge->occurring == 'permanent' ? 'selected' : ''}}>permanent</option>
        </select>
    </div>
    ...
    <div class="mt-4 col-12">
        ...
        <a href="{{ route('admin.charges.index') }}"><div class="btn btn-light">Cancel</div></a>
    </div>
</form>
@endslot
@slot('script')
